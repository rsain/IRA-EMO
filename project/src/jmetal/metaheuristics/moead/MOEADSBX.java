...
public class MOEADSBX extends Algorithm {

  public static enum FunctionType{TCHE, TCHEp, PBI};
   ...
   */
  public MOEADSBX(Problem problem) {
    super (problem) ;
   ...
   * @param problem Problem to solve
   */
  public MOEADSBX(Problem problem, FunctionType functionType) {
    super (problem) ;
  ...
  public SolutionSet execute() throws JMException, ClassNotFoundException {
    ...
    dataDirectory_ = this.getInputParameter("weightsDirectory").toString();
    functionType_ = (FunctionType)this.getInputParameter("functionType"); 
   ...
   // 	System.out.println(lambda_[i][0] + " " + lambda_[i][1]) ;
    ...
    do {
      ...
      for (int i = 0; i < populationSize_; i++) {
        ...
        int res = dc.compare(offSpring[0], offSpring[1]);
  ...
  public void initUniformWeight() {
    if ((problem_.getNumberOfObjectives() == 2) && (populationSize_ <= 300)) {
      ...
      String dataFileName;
      dataFileName = populationSize_ + "W." + problem_.getNumberOfObjectives() + "D";
  ...
  double fitnessFunction(Solution individual, double[] lambda) {
	    ...
	    if (functionType_.compareTo(FunctionType.TCHE)==0) {
	      ...
	      fitness = maxFun;
	    ...
	    else if (functionType_.compareTo(FunctionType.TCHEp)==0) {
	        double maxFun = -1.0e+30;
	        double sumFun = 0.;
	        double rhoFun = 0.001;
	        ...
	        fitness = maxFun + rhoFun*sumFun;
	      } // if
	    
	    else if (functionType_.compareTo(FunctionType.PBI)==0) {	    		    
	        double d1, d2, param_pen;
	        double first_sum, second_sum, temp_value;
	        ...
	        {		
	        	...
	        	first_sum = first_sum + lambda[n]*(Math.abs(individual.getObjective(n) - z_[n]));
	        	second_sum = second_sum + Math.pow(lambda[n],2);	      	
	        }
			
	        d1 = first_sum/Math.pow(second_sum,0.5);
	        
	        first_sum = 0;
	        for (int n = 0; n < problem_.getNumberOfObjectives(); n++) 
	        {	
	        	temp_value = individual.getObjective(n) - (z_[n] + d1*lambda[n]);
	            first_sum = first_sum +  Math.pow(temp_value, 2);	
	        }
	        d2 = Math.pow(first_sum,0.5); 
	        
	        fitness =  d1+ param_pen*d2; 
